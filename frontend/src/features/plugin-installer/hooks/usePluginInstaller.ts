...
const INSTALLATION_STEPS: Omit<InstallationStep, 'status'>[] = [
...
export const usePluginInstaller = () => {
  const [installationState, setInstallationState] = useState<PluginInstallationState>({
    isInstalling: false,
    currentStep: 0,
    steps: INSTALLATION_STEPS.map(step => ({ ...step, status: 'pending' })),
    result: null,
    error: null
  });
  ...
  const installPlugin = useCallback(async (request: PluginInstallRequest): Promise<PluginInstallResponse> => {
    try {
      ...
      const result = await pluginInstallerService.installFromUrl({
        ...request,
      ...
      if (result.status === 'success') {
        ...
        if (errorDetails?.step) {
          ...
          if (result.error?.includes('repository') || result.error?.includes('download')) {
            failedStep = 1;
