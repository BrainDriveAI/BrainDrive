...
export function resumeCreateInputSchema() {
  return {
    type: "object",
...
function renderResumeMarkdown(input) {
  if (typeof input?.resume_markdown === "string" && input.resume_markdown.trim()) return renderResumeTemplateStandard(input.resume_markdown);
...
function renderResumeTemplateStandard(profileMarkdown) {
  ...
  for (const section of parsed.other) appendResumeTemplateSection(lines, section.heading, section.lines);
  return lines.join("\n").replace(/\n{3,}/g, "\n\n").trim();
}

function parseResumeProfileSections(markdown) {
  const sections = [];
  let title;
  let current = null;
...
function isExportableResumeMarkdown(markdown) {
...
function renderResumeMarkdownPdf(markdown) {
  ...
  for (const page of pages) {
    ...
    objects.push(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 4 0 R /F2 5 0 R /F3 5 0 R >> >> /Contents ${contentRef} 0 R >>`);
  ...
  for (let index = 0; index < objects.length; index += 1) {
    offsets.push(bufferByteLength(chunks));
    chunks.push(Buffer.from(`${index + 1} 0 obj\n`, "utf8"), objectBuffer(objects[index]), Buffer.from("\nendobj\n", "utf8"));
  }
  const xrefOffset = bufferByteLength(chunks);
...
function fontFileObject(bytes) {
  return pdfStream(bytes, ` /Length1 ${bytes.length}`, true);
}

function fontDescriptorObject(fontName, fontFileRef) {
  return `<< /Type /FontDescriptor /FontName /${fontName} /Flags 32 /FontBBox [-1000 -300 2000 1200] /ItalicAngle 0 /Ascent 1000 /Descent -300 /CapHeight 700 /StemV 80 /FontFile2 ${fontFileRef} 0 R >>`;
}

function cidToGidMapObject(fontBytes) {
...
function type0FontObject(fontName, descendantRef) {
  return `<< /Type /Font /Subtype /Type0 /BaseFont /${fontName} /Encoding /Identity-H /DescendantFonts [${descendantRef} 0 R] /ToUnicode 3 0 R >>`;
}

function toUnicodeCMapObject() {
  const cmap = [
  ...
  return pdfStream(Buffer.from(cmap, "utf8"), "", true);
}

function pdfStream(stream, extraDictionary = "", compress = false) {
  const output = compress ? deflateSync(stream) : stream;
  return Buffer.concat([
    Buffer.from(`<< /Length ${output.length}${extraDictionary}${compress ? " /Filter /FlateDecode" : ""} >>\nstream\n`, "utf8"),
    output,
    Buffer.from("\nendstream", "utf8"),
  ]);
}
...
function objectBuffer(object) {
  return Buffer.isBuffer(object) ? object : Buffer.from(object, "utf8");
}
...
function renderPdfPages(blocks, fontUsage) {
  ...
  const drawRuns = (runs, x, baseline, fontSize) => {
    let cursor = x;
...
function wrapPdfRuns(runs, maxWidth, fontSize) {
  const lines = [];
  let current = [];
  let currentWidth = 0;
  const pushRun = (run) => {
...
function assertPdfFontCoverage(value, filename) {
...
function readTrueTypeCmap(fontBytes) {
...
function findTrueTypeTable(fontBytes, tag) {
...
function readFormat12Cmap(fontBytes, offset) {
...
function readFormat4Cmap(fontBytes, offset) {
  const glyphs = new Uint16Array(65_536);
