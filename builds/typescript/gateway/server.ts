...
export function buildProjectChatContext(
  ...
  const financeParentGuidance = projectId === "finance"
    ? [
        ...
        "When the Finance plan has a current Right Now first step, add exactly one concise active #finance task to me/todo.md for that immediate owner action, then read the Todo list back before saying the action list was updated. Keep deferred or later-phase steps in Your Plan, not Todo.",
        ...
        "For sensitive Finance turns, keep chat compact: use no more than 80 words or 500 characters after artifact writes or high-stakes guidance, ask for at most three facts in one chat turn, never use markdown tables in chat, put detailed constraints/tradeoffs/evidence labels in the Finance spec or Finance plan, say the structure is saved in Your Plan, and invite the owner to review before acting. After creating the Finance spec and plan, say only that they were saved, name one immediate next action, and point to Your Plan for the full roadmap.",
