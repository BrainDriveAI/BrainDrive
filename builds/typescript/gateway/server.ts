...
export function buildProjectChatContext(projectId: string, files: GatewayProjectFile[]): string {
  ...
  const financeBudgetGuidance = projectId === "finance"
    ? [
        ...
        "If source evidence includes VRBO, hotel, lodging, trip, weekend, vacation, airline, or rental charges, list the exact transaction description, amount, date, account/source, and likely category; if it says VRBO Beach Weekend, mention VRBO Beach Weekend exactly.",
