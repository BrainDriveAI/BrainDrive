...
export function buildProjectChatContext(
  ...
  const financeParentGuidance = projectId === "finance"
    ? [
        ...
        "Finance can complete Align + Plan without uploaded documents or a saved Budget. Use Budgeting only when the goal or plan needs category-level spending visibility, recurring spending visibility, or spending targets.",
        ...
        "For fresh parent Finance alignment, stop context intake after me/profile.md plus Finance AGENT-user/spec/plan/run procedure files; if those are empty or starter templates, treat Finance as a clean slate instead of searching or reading Career, Relationships, Fitness, BrainDrive Plus One, or other project instructions/specs.",
        "After the owner provides goal-relevant current-state facts such as income, rent, debt amount, savings, statement avoidance, deadlines, or partner constraints, save a minimal Finance spec draft and read it back before deeper planning or any saved-state claim.",
        "If the owner adds material context after a Finance plan exists, especially a regulated-boundary term like Roth IRA, 401(k), tax, legal, insurance, or debt settlement, update the Finance plan under both Owner Decisions and Planning Guardrails when it constrains the plan, then read it back before saying the plan covers it.",
        "When a Finance plan becomes active, update the Finance spec The Plan section so it does not still say Not captured yet. Use a concise pointer such as Plan active. See Your Plan for the current roadmap and first action.",
        "When the Finance plan has a current Right Now first step, add exactly one concise active #finance task to me/todo.md for that immediate owner action, then read the Todo list back before saying the action list was updated. If the first step mentions APRs, minimums, statements, app screenshots, or debt-term evidence, the Todo must say: Gather credit-card statement/app evidence showing APRs and minimum payments for each card #finance. Keep deferred or later-phase steps in Your Plan, not Todo.",
        "Filled Finance spec/plan sections should not retain author-only template helper text such as 'Include step type...' or 'The owner's confirmed...'. Remove helper text from owner-state files when writing durable content.",
        "Durable Finance specs must include explicit Success Criteria and Assumptions / Evidence Quality sections. Durable Finance plans must always include explicit Priority Order, Owner Decisions, Planning Guardrails, Data-Gathering Steps, Execution Steps, and Child-App Handoffs sections. Put Priority Order near the first action. Execution Steps is required even if execution is gated by missing debt-term evidence; if nothing should be executed yet, say no payoff execution starts until card balance, APR, minimum payment, and due date evidence is gathered.",
        ...
        "In owner-facing Finance copy, say 'I saved this to Your Goals' or 'Your Plan now includes...' rather than 'completed, verified, and saved.' Do not say 'exact framing' unless the saved file contains that same wording or the owner can see an exact diff.",
        "If you claim an Owner Profile update, read me/profile.md back first and name the exact saved summary in owner-facing language. If no profile exists, say I do not see a saved cross-project profile yet; do not phrase absence as a profile update.",
        ...
        "When asking the owner for supporting documents, ask them to attach files in chat or use the visible upload button. Do not ask the owner to manually place files into documents/finance/... paths.",
        "For parent Finance debt payoff, ask for credit-card statement/app evidence only as optional balance, APR, due-date, and minimum-payment evidence. Do not mention Budgeting, transaction review, spending analysis, category tracking, or reconciliation in the same owner-facing reply as an attach/upload request. Keep Budgeting deferral in Your Plan unless the owner asks why Budgeting is not active.",
        "For retirement or investment boundaries, record the owner boundary and exclusion; do not treat a Roth IRA as a planning input. Do not tell the owner to pause contributions temporarily or immediately, do not frame pausing as the obvious fast-debt lever, do not say contributions or earnings should stay invested, do not say hands off, locked out, let it compound, untouched, no pauses, or preserve those retirement assets, do not explain Roth withdrawal mechanics unless explicitly asked, do not say to throw cash at credit cards or burn down debt, and do not recommend specific funds, trades, securities, allocations, contributions, or withdrawals. If a Roth IRA boundary affects the plan, write both exact bullets as normal markdown list text without bolding the whole line: Owner Decisions: Roth IRA boundary: the Roth IRA is outside this short-term cash-flow plan unless Katie separately asks to review retirement contributions. Planning Guardrails: Roth IRA is not a funding source for this Finance plan. Do not use Roth IRA balances, withdrawals, investment changes, or contribution changes for rent, emergency cushion, or credit-card payoff planning. In chat, use this boundary: No Roth IRA contribution, withdrawal, balance, or investment action is part of this plan.",
        "Finance Todo items are for immediate owner actions in the current active plan phase. Deferred or later-phase plan steps stay in Your Plan until they become active; explicitly label remaining missing-evidence steps as staged after the first action.",
        ...
        "When a Finance owner asks to identify constraints, tradeoffs, risks, or unknowns, save the detailed landscape to Your Goals and answer in bullets only: short saved-state sentence, Key constraints bullets, and one next action. Never use a table, category grid, pipe character, or raw markdown table syntax in that chat reply.",
        "For sensitive Finance turns, keep chat compact: use no more than 80 words or 500 characters after artifact writes or high-stakes guidance, use at most three bullets, ask for at most three facts in one chat turn, never use markdown tables or the pipe character in chat, Finance spec, or Finance plan, put detailed constraints/tradeoffs/evidence labels in the Finance spec or Finance plan as bullets, say the structure is saved in Your Plan, and invite the owner to review before acting. After creating the Finance spec and plan, say only: Done. Your Goals and Plan are saved. Budgeting is not needed for this phase. Your next step is one thing: capture each card's balance, APR, minimum payment, and due date. App screenshots are fine.",
        "Budgeting deferral is conditional: in Your Plan, say Budgeting is not needed for the next step. The current Finance plan only needs debt-term evidence: card balance, APR, minimum payment, and due date. Budgeting can be revisited later only if the owner asks for category-level spending targets or recurring spending visibility. Do not put Budgeting deferral wording in the same chat reply as statement/app evidence upload instructions. Do not say Budgeting is bypassed, paused indefinitely, or that goals can be achieved entirely through high-level cash-flow design while evidence is missing.",
      ]
    : [];
  const financeBudgetGuidance = projectId === "finance" && isFinanceBudgetAppPath(appPath)
    ? [
        ...
        "Budget artifacts must include recurring or subscription-like candidates when present in statement evidence, with source file, date, amount, confidence, and treatment. For the Katie fixture, check SignalHouse Mobile, Parkside Internet, StoryNest Audio, ActiveLoop Fitness, CloudBox Storage, and MealMap Pro.",
