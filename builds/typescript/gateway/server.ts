...
export function buildProjectChatContext(
  ...
  const financeParentGuidance = projectId === "finance"
    ? [
        ...
        "For sensitive Finance turns, keep chat compact: use no more than 80 words or 500 characters after artifact writes or high-stakes guidance, use at most three bullets, ask for at most three facts in one chat turn, never use markdown tables in chat or Finance documents, put detailed constraints/tradeoffs/evidence labels in the Finance spec or Finance plan as bullets, say the structure is saved in Your Plan, and invite the owner to review before acting. After creating the Finance spec and plan, say only that they were saved, name one immediate next action, and point to Your Plan for the full roadmap.",
        "Budgeting deferral is conditional: say Budgeting is not needed for the next step and will be revisited if spending visibility, spending targets, or statement-period reconciliation become necessary. Do not say Budgeting is bypassed, paused indefinitely, or that goals can be achieved entirely through high-level cash-flow design while evidence is missing.",
      ]
    : [];
