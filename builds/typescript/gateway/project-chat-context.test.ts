...
describe("project chat context", () => {
  it("includes current project files and warns against stale delete claims", () => {
    ...
    expect(context).toContain("Use Budgeting only when the goal or plan needs category-level spending visibility, recurring spending visibility, or spending targets");
    ...
    expect(context).toContain("Gather credit-card statement/app evidence showing APRs and minimum payments for each card #finance");
    ...
    expect(context).toContain("Do not mention Budgeting, transaction review, spending analysis, category tracking, or reconciliation");
    ...
    expect(context).toContain("After creating the Finance spec and plan, say only: Done. Your Goals and Plan are saved.");
    ...
    expect(context).toContain("The current Finance plan only needs debt-term evidence: card balance, APR, minimum payment, and due date");
    expect(context).toContain("Do not say Budgeting is bypassed, paused indefinitely");
    expect(context).not.toContain("statement-period");
