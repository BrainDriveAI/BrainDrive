...
export class ModelCompatibilityRegistry {
  ...
  require(providerProfileId: string, modelId: string, purpose: InferencePurpose): CompatibilityEntry {
    ...
    const entry = this.entries.find((candidate) => candidate.provider_profile_id === providerProfileId && candidate.model_id === modelId && candidate.purpose === purpose && candidate.compatible && candidate.prompt_policy_id === promptPolicy.id && candidate.prompt_policy_version === promptPolicy.version && candidate.output_schema_id === PURPOSE_OUTPUT_SCHEMAS[purpose] && candidate.fixture_corpus_digest === conformanceCorpusDigest(purpose));
