...
describe("Brief Builder first-party package and registration", () => {
  it("binds exact reviewed capabilities, purpose, adapter, resource, and all platform targets", async () => {
    ...
    expect(registration.capability_registrations.map((item) => item.key.name)).toEqual(["brief.records.read", "brief.records.write", "brief.approvals.confirm", "app.inference.request", "web.search", "web.read"]);
    expect(registration.inference_purpose_registrations).toMatchObject([{ key: { purpose_id: "brief.generate", version: 1 }, prompt_policy_id: "brief.generate.fixed.v2", validation_policy_id: "brief.grounding.v1" }]);
    ...
    try {
      ...
      expect(verified).toMatchObject({ manifest: { app_id: "ai.braindrive.brief-builder", primary_resource: { uri: "ui://brief-builder/main" }, requested_capabilities: expect.arrayContaining([{ name: "web.search", version: 1 }, { name: "web.read", version: 1 }]), requested_inference_purposes: [{ purpose_id: "brief.generate", version: 1 }], capability_dependencies: [{ operation_id: "web.search@1", requirement: "required", unavailable_behavior: "block_activation", provider_selection: "owner_or_admin_policy", silent_install_or_switch: false }, { operation_id: "web.read@1", requirement: "required", unavailable_behavior: "block_activation", provider_selection: "owner_or_admin_policy", silent_install_or_switch: false }] }, trust: { executable_allowed: true } });
