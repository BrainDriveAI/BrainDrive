...
This directory is the generic capability control plane used by installed apps. It enforces named authority, lifecycle grants, operation identity, confirmation, and safe host services without placing app-specific workflow or inference policy in BrainDrive core. Resume Builder is one installed consumer; it owns all twelve Resume inference programs inside its signed package and never receives memory roots, raw filesystem tools, or provider credentials.
