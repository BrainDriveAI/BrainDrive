...
*Procedure for the follow-up session: keeping Career current after the first plan, from the owner's updates in conversation. This file drives `journal.md` (Your Journal), exactly as `run-interview.md` drives `spec.md` and `run-planning.md` drives `plan.md`.*
...
Read the likely career blocker from what the owner actually does, not what they intend: the conversation they keep not having (manager, recruiter, ask for the raise), evidence they keep not gathering, a decision they keep re-deliberating instead of advancing (stay/leave, apply/don't), confidence or "I haven't done enough" stories that don't match their own record, burnout signals, or waiting for permission someone else was never going to give. Career blockers are usually either avoidance of a specific conversation or a decision missing one concrete piece of evidence — figure out which before proposing anything. Keep the next step small and concrete ("book the twenty-minute chat," "write down the three wins," "ask one person in that role"), and in charged situations (burnout, layoff, performance trouble, workplace conflict) respond with respect — no blame, minimizing, or false cheerfulness — while still moving toward a useful next step.
...
Insert a dated entry at the top of the history in `documents/career/journal.md` (following the Preservation Rule above):
...
Review history for recurring work blockers, confidence patterns, role-fit signals, follow-up gaps, decision evidence accumulating for or against the plan's direction, and burnout signals; surface them as owner-calibrated hypotheses, never verdicts. Stay inside the career trust boundary on every entry and every reflection:

- Store what managers, recruiters, colleagues, and interviewers said as **owner-provided facts only** — capture what the owner reports in their words, and never invent, embellish, or read another person's intent, mood, or plans into an entry.
- Do not present certainty about employers, hiring outcomes, promotions, the job market, or salary truth. Label uncertainty plainly; decisions rest on owner-controlled next steps, not predictions.
- No shame about career struggles. Burnout, a stalled search, a botched interview, or avoidance of a hard conversation get named plainly and worked as blockers, never moralized.

## Close-Out

Done when: every journal-worthy update the owner shared is captured in `journal.md` (insert-only history preserved, newest first) and the owner was told what you recorded — if nothing journal-worthy surfaced, nothing was written (a view-only or advice-only session may rightly write nothing); any owner correction or removal was applied as asked; any parent change was owner-approved, concise, and recorded in the journal; and the owner knows what happened and the next useful step. Then return to Career scope.
...
It is not an ATS, application tracker, job-search pipeline, resume or narrative builder, offer-negotiation tool, or LinkedIn-style profile; it does not ingest employer, email, or calendar data. Resume drafting, job-search organization, and negotiation execution are future external tool calling (V.1.1), not this journal — entries capture the decision and follow-up context, not the drafts.
