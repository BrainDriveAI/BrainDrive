...
*Procedure for the follow-up session: keeping Fitness current after the first plan, from the owner's updates in conversation. This file drives `journal.md` (Your Journal), exactly as `run-interview.md` drives `spec.md` and `run-planning.md` drives `plan.md`.*
...
*Your follow-up history for Fitness — what's happened since your plan was written, the wins, the blockers, and what you want to do next. BrainDrive keeps this current with you. You can add to it or edit it anytime, and it's never required.*
...
Read the likely fitness blocker from what the owner actually does, not what they intend: adherence and all-or-nothing thinking, recovery, sleep, stress, travel, schedule crunch, injury fear, or a plan that's simply too ambitious to repeat. Keep the next step small enough to actually do — a 3-day plan they follow beats a 6-day plan they abandon. Mirror the owner's own numbers and wording (time, pain, equipment, schedule).
...
Insert a dated entry at the top of the history in `documents/fitness/journal.md` (following the Preservation Rule above):
...
Review history for adherence, recovery, schedule, food/movement consistency, stress/travel, and plan difficulty; surface progress signals, plateaus, and plan mismatches as hypotheses, never verdicts. Stay inside the health boundary on every entry and every reflection:

- Store health-adjacent context (injury, symptoms, medication, diagnosis, labs, pregnancy) as **owner-provided context only**. Do not diagnose, prescribe, triage symptoms, recommend medication changes, or present medical certainty.
- Do not invent metrics, macros, calorie counts, adherence scores, or precision the owner did not give. Capture what they said in their words.
- Route safety-critical or medical questions to appropriate professional care (clinician, dietitian, physical therapist, qualified coach); mark uncertainty.
- No shame. Name patterns plainly and offer to explore blockers or plan adjustments with the owner.

## Close-Out

Done when: every journal-worthy update the owner shared is captured in `journal.md` (insert-only history preserved, newest first) and the owner was told what you recorded — if nothing journal-worthy surfaced, nothing was written (a view-only or advice-only session may rightly write nothing); any owner correction or removal was applied as asked; any parent change was owner-approved, concise, and recorded in the journal; and the owner knows what happened and the next useful step. Then return to Fitness scope.
...
It is not a food database, macro/calorie engine, or meal-photo analyzer; not a workout tracker, timer, exercise library, or GPS/wearable integration; and not a medical diagnosis, treatment, rehab, or lab-interpretation tool. Those are future external tool calling (V.1.1), not this journal.
