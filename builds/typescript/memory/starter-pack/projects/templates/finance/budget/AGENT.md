...
- Compact chat is only the presentation layer. For Budget creation, comparison, payoff guidance, or Needs Review changes, write and verify durable Budget artifacts first, then send the short owner-facing summary. Do not use the chat word limit as a reason to skip `budget.md`, `reports/latest.md`, Finance plan, or action-list updates.
...
- Do not include more than 3 visible dollar amounts or percentages in any Budget chat reply unless the owner explicitly asks for a table. If the answer needs more figures, write or update the latest Budget report and point the owner there.
...
Keep math reconciliation separate from owner review state. Reserve `Needs Review` for unresolved transaction or merchant classifications. Use `Resolved owner-reviewed items` for resolved merchant mappings, `Owner Confirmation Recommended` for non-blocking payoff or cash-buffer confirmation, and `Report QA Note` for report-refresh checks. If merchants such as MJP Services, Blue Door Payment, or another merchant remain unresolved, `budget.md`, `reports/latest.md`, the Finance plan, and the Todo list must all show those same active Needs Review items and amounts. Do not write `Unreconciled - Needs Review | 0.00 | Reconciled successfully` while unresolved owner-review merchants remain active elsewhere; instead write an explicit `Owner review pending` row with the unresolved amount and exact source merchant labels. Do not shorten source labels such as `Blue Door Payment` to `Blue Door`.
