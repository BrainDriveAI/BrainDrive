...
*Procedure for the follow-up session: keeping New Project current after the first plan, from the owner's updates in conversation. This file drives `journal.md` (Your Journal), exactly as `run-interview.md` drives `spec.md` and `run-planning.md` drives `plan.md`.*
...
~~~md
# Your New Project Journal

*Your follow-up history for New Project — what's happened since your plan was written, the wins, the blockers, and what you want to do next. BrainDrive keeps this current with you. You can add to it or edit it anytime, and it's never required.*
...
Read the likely blocker from what the owner actually does with their created pages, not what they intend: a page that never gets opened (a signal about fit or friction, never a moral failing), a spec or plan too vague to act on, work landing in the wrong page, two pages overlapping, or a page that was really a one-off task or an idea that wasn't ready. The fix is usually either sharpening the created page (a clearer next step, a tighter scope) or changing its lifecycle (merge, reroute, archive, keep lightweight) — figure out which before proposing anything, and treat archiving an unused page as a win, not a failure.
...
Insert a dated entry at the top of the history in `documents/new-project/journal.md` (following the Preservation Rule above):
...
Review history for created-page usefulness and friction, duplicate or misplaced pages, lifecycle signals (deepen, merge, archive, keep lightweight), and repeated needs for heavier execution; surface them as owner-calibrated hypotheses, never verdicts. Stay inside the New Project boundary on every entry and every reflection:

- This journal captures how created pages are **working** — the created page's own content (its goals, plans, progress) lives in that page's files, not here. If an update is really about the created page's subject matter, route it there.
- Never rewrite a created page from journal signals alone. Propose the change (refine, merge, reroute, archive) and get owner approval before touching any created page's files — and keep approved changes concise.
- A repeated need for heavier execution is a future external-tool candidate (V.1.1): capture it as a signal, without starting build work or promising the capability.
- More pages is not more progress. When a pattern points at sprawl — pages created and abandoned, overlapping scopes — name it plainly and help the owner consolidate.

## Close-Out

Done when: every journal-worthy update the owner shared is captured in `journal.md` (insert-only history preserved, newest first) and the owner was told what you recorded — if nothing journal-worthy surfaced, nothing was written (a view-only or advice-only session may rightly write nothing); any owner correction or removal was applied as asked; any parent change was owner-approved, concise, and recorded in the journal; and the owner knows what happened and the next useful step. Then return to New Project scope.
...
It is not a hidden build backlog, a page-management dashboard, or an app generator; it does not create build or prioritization commitments, silently rewrite created pages, or store created-page domain content. It records owner-provided evidence about whether created pages are useful — nothing more.
