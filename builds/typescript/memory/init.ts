...
function fallbackProjectTemplateContent(projectName: string, fileName: string): string {
  ...
  if (fileName === "budget/AGENT.md") {
    return [
      ...
      "",
      "Internal file paths are for tool use only. In normal owner-facing replies, use product labels such as saved Budget, latest Budget report, Budget statements, Finance goals, Finance plan, and Todo list. Do not mention `AGENT.md`, procedure files, rules files, or raw markdown filenames unless the owner explicitly asks for exact technical paths.",
      "",
      "## App-Level Flow",
      "",
      ...
      "",
      "After accepting uploads, propagate state to the Budget statement checklist, Finance goals, Finance plan, and Todo list so completed statement gathering is not left as active missing work.",
      "",
  ...
  if (fileName === "budget/create.md") {
    return [
      ...
      "",
      "When credit-card debt appears in the statements, add a `Debt Payoff Priority` section to `budget.md` and, when refreshed, `reports/latest.md`. Required fields: card name, balance, APR, minimum payment, priority rank, and payment instruction. The owner-facing summary must explicitly say which card has the higher APR and where extra payoff money goes. Example wording: `Northbridge Rewards Visa is the higher-APR card at 22.49%, so pay Summit Trail's $117.00 minimum and direct all extra payoff money to Northbridge first while also paying Northbridge's $139.00 minimum.` Adapt names, rates, and minimums to the actual statements.",
      "",
      ...
      "",
      "If the first Budget reply would exceed a compact summary, say the detailed category table is in the saved Budget or latest Budget report instead of pasting it into chat. For debt payoff, show the named APR priority block in chat even when the full math is in the artifact.",
      "",
