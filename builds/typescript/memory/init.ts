...
function fallbackProjectTemplateContent(projectName: string, fileName: string): string {
  ...
  if (fileName === "budget.md") {
    return [
      ...
      "",
      "During saved-budget comparisons, read this file for limits but do not write, edit, or delete it unless the owner explicitly asks to revise the budget. Put comparison findings in reports/latest.md instead.",
      "",
  ...
  if (fileName === "reports/latest.md") {
    return [
      ...
      "## Instructions",
      "",
      "Use this report as the write target for saved-budget comparisons. Do not write, edit, or delete budget.md unless the owner explicitly asks to revise the saved budget.",
      "",
      ...
      "",
      "Build a source evidence ledger before summarizing. Carry named, new, unusual, material, excluded, or needs-review source items into the final report, and do not later describe a found source item as absent without re-reading and citing the checked source files/date ranges.",
      "",
      ...
      "",
      "Extract owner-requested merchant, item, trip, bill, and transaction names from the current request and recent follow-ups, then complete Owner-Requested Items Audit. If an item is found, include the exact source description in the audit and also include it in a treatment section. If not found, say which source files/date ranges were checked. If uncertain, mark it Needs Review instead of omitting it.",
      "",
      "Include New Or Unbudgeted Items for new, unusual, travel/lodging/vacation, entertainment, shopping, or unclear merchants. Use exact statement descriptions so the owner can trace each item back to source evidence.",
      "",
      "## Summary",
      "",
      "- Overall status:",
