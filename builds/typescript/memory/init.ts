...
function fallbackProjectTemplateContent(projectName: string, fileName: string): string {
  ...
  if (fileName === "budget/create.md") {
    return [
      ...
      "",
      "When credit-card debt appears in the statements, add a `Debt Payoff Priority` section to `budget.md`, the parent Finance plan, and, when refreshed, `reports/latest.md`. Required fields: card name, balance, APR, minimum payment, priority rank, concrete monthly extra-payment target, total priority-card target payment, and payment instruction. The owner-facing summary must explicitly say which card has the higher APR and where the extra payoff amount goes. Example wording: `Northbridge Rewards Visa is the higher-APR card at 22.49%, so pay Summit Trail's $117.00 minimum and send a $250.00 monthly extra payment to Northbridge first while also paying Northbridge's $139.00 minimum.` Adapt names, rates, minimums, and extra amount to the actual statements.",
      "",
  ...
  if (fileName === "budget/compare.md") {
    return [
      ...
      "Separate completion state from follow-up state in the final reply: created or updated artifacts, open owner decisions or assumptions, and active Todo items that still remain.",
      "",
      "Keep the chat reply scan-friendly. Use bullets for the owner-facing summary and keep detailed variance tables in the saved report artifact unless the owner explicitly asks to see a table in chat. For comparison replies, stay between 100-160 words and no more than 5 visible dollar amounts or percentages unless the owner explicitly asks for a table. Do not send malformed markdown, dangling emphasis markers, repeated emphasis markers, concatenated category words, jammed amounts, or adjacent merchant names without line breaks.",
      "",
