...
function fallbackProjectTemplateContent(projectName: string, fileName: string): string {
  ...
  if (fileName === "run-planning.md") {
    return [
      ...
      "",
      "Credit-card statement PDFs can remain parent Finance data gathering only when the purpose is balance, APR, due-date, or minimum-payment evidence. Say the files are optional evidence for those debt facts only, not Budget setup, Budget materials, transaction review, spending analysis, or statement-period reconciliation unless the owner chooses a Budgeting handoff.",
      "",
      ...
      "## Done Criteria",
      "",
      "`plan.md` names the first step, roadmap, destination, and remaining blockers without copying full reports into the plan. If only one active Todo is appropriate, remaining missing-evidence steps are explicitly labeled later, deferred, or staged after the first action.",
      "",
      "## After Running",
      "",
      "Update `plan.md`, add concrete todos only for immediate owner actions in the current active plan phase, and return to the parent Finance scope. Deferred or later-phase plan steps stay in Your Plan and should not become Todo list items until they become the next active owner action. In chat after artifact writes, stay under 120 words or 800 characters, summarize only artifact changes and the next step, never say completed/verified/exact framing unless the owner can see exact verification, never say saved Budget or Budget materials unless Budgeting wrote and read back a Budget artifact, and invite the owner to review Your Plan before acting when payoff or retirement-boundary guidance changed. Budgeting deferral is conditional: not needed for the next step, revisit if spending visibility, spending targets, or statement-period reconciliation become necessary. Do not say Budgeting is bypassed, paused indefinitely, or that goals can be achieved entirely through high-level cash-flow design while evidence is missing.",
      "",
      "## What This Procedure Is Not",
      "",
  ...
  if (fileName === "budget/AGENT.md") {
    return [
      ...
      "",
      "Keep math reconciliation separate from owner review state. Reserve `Needs Review` for unresolved transaction or merchant classifications. Use `Resolved owner-reviewed items` for resolved merchant mappings, `Owner Confirmation Recommended` for non-blocking payoff or cash-buffer confirmation, and `Report QA Note` for report-refresh checks. If merchants such as MJP Services or Blue Door Payment remain unresolved, `budget.md`, `reports/latest.md`, the Finance plan, and the Todo list must all show those same active Needs Review items and amounts. Do not write `Unreconciled - Needs Review | 0.00 | Reconciled successfully` while unresolved owner-review merchants remain active elsewhere; instead write an explicit `Owner review pending` row with the unresolved amount and merchant names.",
      "",
      ...
      "",
      "For every Budget chat turn, stay between 100-160 words unless the owner explicitly asks to see a table or detailed breakdown in chat. Use one short answer to the owner's question, one saved-artifact pointer, and one next action or clarifying question. Do not include more than 3 visible dollar amounts or percentages unless the owner explicitly asks for a table.",
      "",
  ...
  if (fileName === "budget/create.md") {
    return [
      ...
      "",
      "Separate math reconciliation from unresolved owner-review state. Reserve `Needs Review` for unresolved transaction or merchant classifications. If MJP Services, Blue Door Payment, or another merchant still needs the owner to classify it, include an `Owner review pending` row in `budget.md` with the active amount and exact source merchant labels, even when the math subtotal difference is $0.00. Once resolved, replace active review language with `Resolved owner-reviewed items`, keep exact source merchant labels visible, and move non-blocking payoff or cash-buffer questions to `Owner Confirmation Recommended`. The same active Needs Review items must remain visible in `reports/latest.md`, the parent Finance plan, and `me/todo.md` until resolved. Do not shorten source labels such as `Blue Door Payment` to `Blue Door`.",
      "",
