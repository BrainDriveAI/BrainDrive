...
function fallbackProjectTemplateContent(projectName: string, fileName: string): string {
  ...
  if (fileName === "run-planning.md") {
    return [
      ...
      "## After Running",
      "",
      "Update `plan.md`, add concrete todos only when there is an actual next action, and return to the parent Finance scope. In chat after artifact writes, stay under 120-160 words, summarize only artifact changes and the next step, and invite the owner to review Your Plan before acting when payoff or retirement-boundary guidance changed.",
      "",
      "## What This Procedure Is Not",
      "",
