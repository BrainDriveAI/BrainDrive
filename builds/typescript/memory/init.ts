...
function fallbackProjectTemplateContent(projectName: string, fileName: string): string {
  ...
  if (fileName === "budget/create.md") {
    return [
      ...
      "",
      "Separate math reconciliation from unresolved owner-review state. If MJP Services, Blue Door Payment, or another merchant still needs the owner to classify it, include a `Needs Review` section and an `Owner review pending` row in `budget.md` with the active amount and exact merchant labels, even when the math subtotal difference is $0.00. The same active Needs Review items must remain visible in `reports/latest.md`, the parent Finance plan, and `me/todo.md` until resolved. Do not shorten source merchant labels such as `Blue Door Payment` to `Blue Door`.",
      "",
  ...
  if (fileName === "budget/statements/README.md") {
    return [
      ...
      "",
      "Post-upload chat must stay in receipt phase. Keep it to 35-70 words and no more than 3 short sentences: confirm the batch count, state that the files are ready for Budget analysis, and give one next action. Do not enumerate filenames or present statement-derived balances, APRs, interest charges, income calculations, category totals, rent math, Needs Review merchants, payoff recommendations, or tool/process narration until the owner asks for the Budget plan, comparison, or report.",
      "",
    ].join("\n");
  }
