...
async function setup(dutyOnly = false, includeUnusedContext = false, evidenceLimited = false, hooks: StoreHooks = {}) {
  ...
  const generation = { title: "Synthetic Owner", statements, section_order: ["experience"], omissions: [] };
  ...
  const written = await service.writeDefinition({ ...definitionInput(factId), status: "proposed", title: generation.title, statements, prompt_policy_version: RESUME_PROMPT_POLICY_VERSION, strategy_binding: strategyBinding, generation_result: generation }, authority("resume.definitions.write"));
