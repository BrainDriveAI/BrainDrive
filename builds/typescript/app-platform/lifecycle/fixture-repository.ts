...
export const MODERN_FIXTURE_VERSION = "4.2.21" as const;
...
const server = http.createServer((request, response) => {
  ...
  request.on("end", () => {
    ...
    if (message.method === "tools/call" && message.params?.name === "app.actions.plan") { try { const result=planResumeAction(message.params.arguments,{exportByteDelivery:"runtime_reference",createExportBytesReference}); send(response,message.id,{resultType:"complete",content:[],structuredContent:result,_meta:{ui:{visibility:["model"]}},isError:false}); } catch (error) { const failureMessage=error instanceof Error && error.message.startsWith("PDF export cannot include unsupported characters:") ? error.message : "Installed app action planning failed"; response.writeHead(409,{"content-type":"application/json"}); response.end(JSON.stringify({jsonrpc:"2.0",id:message.id,error:{code:-32602,message:failureMessage}})); } return; }
