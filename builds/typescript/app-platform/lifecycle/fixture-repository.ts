...
const server = http.createServer((request, response) => {
  ...
  request.on("end", () => {
    ...
    if (message.method === "initialize") { send(response, message.id, {protocolVersion:"2026-07-28",capabilities:{tools:{listChanged:false},resources:{listChanged:false}},serverInfo:{name:"resume-builder-fixture",version:"3.0.0"},_meta:{"io.modelcontextprotocol/ui":{version:"2026-01-26"}}}); return; }
    if (message.method === "resources/list") { send(response, message.id, {resources:[{uri:"ui://resume-builder/main",name:"Resume Builder",title:"Resume Builder",description:"Sandboxed owner resume workflow",mimeType:"text/html;profile=mcp-app",size:Buffer.byteLength(appHtml),_meta:{"io.modelcontextprotocol/ui":{version:"2026-01-26"},cachePolicy:"immutable_package_digest"}}]}); return; }
    if (message.method === "resources/read" && message.params?.uri === "ui://resume-builder/main") { send(response, message.id, {contents:[{uri:"ui://resume-builder/main",mimeType:"text/html;profile=mcp-app",text:appHtml,_meta:{"io.modelcontextprotocol/ui":{version:"2026-01-26"},cachePolicy:"immutable_package_digest"}}]}); return; }
    if (message.method === "tools/list") { send(response, message.id, {tools:[{name:"fixture.status",description:"Return the fixture host status",inputSchema:{type:"object",properties:{},additionalProperties:false},_meta:{"io.modelcontextprotocol/ui":{visibility:["app"]}}}]}); return; }
    if (message.method === "tools/call" && message.params?.name === "fixture.status") { send(response, message.id, {content:[{type:"text",text:"Fixture ready",annotations:{audience:["user"],priority:1}},{type:"resource_link",name:"resume-ui",uri:"ui://resume-builder/main",mimeType:"text/html;profile=mcp-app",size:Buffer.byteLength(appHtml),_meta:{visibility:"app"}},{type:"resource",resource:{uri:"ui://resume-builder/state",mimeType:"application/json",text:"{\\\"ready\\\":true}",_meta:{revision:1}}}],structuredContent:{ready:true,version:"3.0.0"},_meta:{"io.modelcontextprotocol/ui":{resourceUri:"ui://resume-builder/main",visibility:["app"]}},isError:false}); return; }
