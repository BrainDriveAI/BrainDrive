...
export async function createAppLifecycle(input: { memoryRoot: string; hostVersion: string; stateRoot?: string; target?: AppLifecycleRuntimeTarget }): Promise<AppLifecycleService> {
  ...
  const service = new AppLifecycleService({
    ...
    ownerDataRoot: path.join(input.memoryRoot, "apps", "resume-builder"),
