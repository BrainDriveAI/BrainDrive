...
export const BridgeMessageSchema = z.discriminatedUnion("type", [
  ...
  BridgeBaseSchema.extend({ type: z.literal("capability.call"), payload: z.object({ capability: z.string().min(1).max(256), input: z.record(z.string(), z.unknown()), token_id: OpaqueIdSchema }).strict() }).strict(),
  ...
  BridgeBaseSchema.extend({ type: z.literal("host.action"), payload: z.object({ action: z.enum(["open_link", "copy_to_clipboard", "navigate_settings", "resize"]), value: z.string().max(2_048), token_id: OpaqueIdSchema }).strict() }).strict(),
