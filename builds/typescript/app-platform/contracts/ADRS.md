...
The BrainDrive release authority owns a signed, independently versioned monotonic revocation list. Refresh is attempted hourly and diagnostics mark it stale after 24 hours. Only a valid higher sequence, or identical content at the current sequence, may update the atomic cache. Every cached explicit revocation remains binding indefinitely. A stale non-matching list remains installable and executable for immutable BrainDrive-reviewed packages as an owner-visible diagnostic; it must not force app developers to republish unchanged artifacts. Online catalog freshness belongs to the BrainDrive-controlled feed, not to per-app release churn. An explicit digest or version-range match fails closed and requires stop, token revocation, and quarantine.
...
## ADR-APP8-001 — Generic first-party contracts do not create executable authority

**Status:** Accepted implementation decision for Spec 08 Milestone 1 on 2026-08-12. The Product Owner accepted Brief Builder as the later second proof app and the conservative resource policy in the attributable implementation instruction. Neither decision enables later-milestone behavior here.

The authoritative registry key is a canonical verified app ID. A distinct canonical host route key maps one-to-one to it. Both use strict ASCII lower-case forms; exact, case-folded, or Unicode-normalized collisions reject registry construction. Publisher identity must be a canonical prefix of its app identity. The registry is host-owned, pure, deterministic, and side-effect free. It accepts reviewed declarative registrations, builds exact indexes, joins an already verified package's source/descriptor/manifest identity and digests, verifies every requested capability/purpose/data-contract binding, and returns a recursively frozen descriptor with a canonical digest.

Generic manifest revision 2 contains bounded presentation metadata, immutable package resources, compatibility facts, a primary `ui://` resource, and capability/purpose requests. It contains no handler, module/import, host policy, or adapter selector. Host registration descriptors use bounded opaque binding identities for separately reviewed composition; no manifest value becomes an import or executable host-code path. The existing Resume revision-1 manifest remains parseable only through its explicit compatibility/migration schema and cannot accept a second app identity. Active package verification, lifecycle records, routes, MCP dispatch, Apps UI, persistence migration, Brief Builder code, and distribution remain unchanged in this milestone.

This is an internal first-party contract. It creates no dynamic registration API, package loader, starter kit, public SDK/ABI, publisher onboarding, marketplace, or third-party compatibility claim.

## ADR-APP8-002 — macOS extends signed first-party package authority without invalidating existing packages

**Status:** Accepted implementation decision on 2026-08-13 for the native macOS desktop target and both reviewed first-party apps.

`desktop_macos_universal` is the third accepted packaged-Node target, bound exactly to `os: macos`, `architecture: universal`, loopback transport, and a matching signed `desktop_macos` source entry. Docker Linux x64 and desktop Windows x64 remain required and unchanged. The macOS artifact/source pair is optional at schema revision 1 and manifest revision 2 so previously signed two-target packages remain parseable; a macOS install still fails closed unless both source and manifest authorize the target. New synthetic Resume Builder and Brief Builder fixtures declare all three targets. This decision supersedes only ADR-RB-003's statement that macOS is unclaimed; it does not alter that ADR's trust, canonicalization, revocation, or retention decisions.

## ADR-APP8-003 — App-specific inference behavior belongs to the installed app

**Status:** Accepted by the Product Owner on 2026-08-15. This supersedes descriptions of Resume Builder or Brief Builder as BrainDrive first-party product modules and supersedes the inference-policy placement in ADR-APP8-001 for newly migrated contracts.

BrainDrive owns generic app lifecycle, authority, isolation, credential mediation, cancellation, bounded provider execution, and safe result envelopes. An installed app owns every behavior needed to make that app work: prompts, output schemas, semantic issue identifiers, candidate validation, repair instructions, and deterministic fallback. BrainDrive must not load or execute those policies when the app is not installed and active.

Inference contract version 2 implements this split. The verified app process exposes private model-visible prepare and adjudicate tools on its existing authenticated connection. The sandbox cannot call those tools. The generic host executor permits at most two sequential structured no-tools calls and never sends credentials, endpoints, or provider-selection authority to the app. As of 2026-08-16, all twelve Resume Builder purposes use this contract and contract version 1 is rejected; the old Resume-specific broker, snapshot, and adapter modules are retained only as offline historical conformance evidence and have no production caller.
