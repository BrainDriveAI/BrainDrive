...
import { resolveAppCapability, type AppCapabilityName, type AppDataCapability } from "../../app-capabilities/registry.js";
import { AppInferenceCapability } from "../../app-inference/capability.js";
...
export class AppMcpHost {
  ...
  constructor(
    ...
    this.clientFactory = options.clientFactory ?? ((connection) => {
      const identity = identityForRuntime(connection);
  ...
  private readonly appInference?: AppInferenceCapability;
  ...
  cancelAppsBridgeRequest(sessionId: string, operationId: string): boolean {
    ...
    if (!session || !this.viewRegistry.isCurrentSession(sessionId) || !session.appsBridgeOperations.has(operationId)) return false;
  ...
  async handleBridge(sessionId: string, rawMessage: unknown, context: { origin: string; sourceMatches: boolean }): Promise<{ status: "ready" } | { status: "completed"; result: CompleteMcpResult } | { status: "capability_completed"; result: unknown }> {
    ...
    if (message.type === "operation.cancel") {
      ...
      return { status: "capability_completed", result: { cancelled: this.appInference.cancel(session.installationId, target, inferenceIdempotencyKey(target)) } };
    }
  ...
  close(sessionId: string): boolean {
    ...
    const closedView = this.viewRegistry.close(sessionId);
    if (!closedView.closed) return false;
    for (const operationId of session.inferenceOperations) this.appInference?.cancel(session.installationId, operationId, inferenceIdempotencyKey(operationId));
  ...
  private async requireSession(sessionId: string): Promise<SessionRecord> {
    ...
    if (Date.parse(session.expiresAt) <= this.now()) {
      ...
      this.viewRegistry.close(sessionId);
    ...
    if (!this.viewRegistry.isCurrentSession(sessionId)) {
      this.sessions.delete(sessionId);
      this.lifecycle.dependencies.tokenBroker.revokeView(session.viewId);
