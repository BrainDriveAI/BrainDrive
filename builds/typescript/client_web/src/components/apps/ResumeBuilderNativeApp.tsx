...
export default function ResumeBuilderNativeApp({ onClose, onOpenSettings }: { onClose: () => void; onOpenSettings?: () => void }) {
  ...
  return <div className="flex min-h-0 flex-1 bg-bd-bg-chat" data-testid="resume-builder-native-app">
    ...
    <ChatPanel activeConversationId={conversationId} draftKey="resume-builder" isEmpty={conversationId === null} introProjectId="resume-builder" messageMetadata={{ resume_builder: true }} onConversationComplete={completeConversation} onConversationMissing={clearMissingConversation} onOpenSettings={onOpenSettings} onSendMessage={() => selectView("chat")} contentOverride={view === "chat" ? undefined : <section className="flex h-full min-w-0 flex-1 flex-col bg-bd-bg-chat text-bd-text-primary">
      ...
      <main className="min-h-0 flex-1 overflow-y-auto px-4 py-6 sm:px-6"><div className="mx-auto w-full max-w-[780px]"><h2 className="font-heading text-2xl font-semibold text-bd-text-heading">{title}</h2>{view === "resume" ? <p className="mt-1 text-sm text-bd-text-muted">Rendered from Your Resume Profile. Edit the profile to change the source.</p> : null}{documentError ? <p className="mt-4 text-sm text-bd-danger">{documentError}</p> : isEditing ? <><textarea aria-label={`Edit ${view}`} value={draftContent} onChange={(event) => setDraftContent(event.target.value)} className="mt-5 min-h-[420px] w-full rounded-2xl border border-bd-border bg-bd-bg-secondary px-5 py-4 font-mono text-sm leading-7 text-bd-text-primary outline-none focus:border-bd-amber/60" /><div className="mt-3 flex justify-end gap-2"><button type="button" onClick={() => setIsEditing(false)} className="rounded-md px-3 py-2 text-sm text-bd-text-secondary hover:bg-bd-bg-hover">Cancel</button><button type="button" onClick={() => void saveDocument()} disabled={isSaving} className="rounded-md bg-bd-amber px-3 py-2 text-sm font-medium text-bd-bg-primary disabled:opacity-60"><Save className="mr-1 inline" size={14} />{isSaving ? "Saving…" : "Save"}</button></div></> : view === "resume" ? <ResumePreview content={documentContent} /> : documentContent ? <div className="prose-bd mt-5 max-w-full text-[15px] leading-7 text-bd-text-primary"><MarkdownContent content={documentContent} /></div> : <p className="mt-5 text-sm text-bd-text-muted">Loading…</p>}</div></main>
    </section>} />
  </div>;
}
