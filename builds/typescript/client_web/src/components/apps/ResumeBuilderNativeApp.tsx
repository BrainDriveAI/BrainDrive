...
export default function ResumeBuilderNativeApp({ onClose, onOpenSettings }: { onClose: () => void; onOpenSettings?: () => void }) {
  ...
  return <div className="flex min-h-0 flex-1 bg-bd-bg-chat" data-testid="resume-builder-native-app">
    <Navigation />
    {view === "chat" ? <ChatPanel activeConversationId={conversationId} draftKey="resume-builder" isEmpty={conversationId === null} introProjectId="resume-builder" messageMetadata={{ resume_builder: true }} onConversationComplete={completeConversation} onOpenSettings={onOpenSettings} /> : <section className="flex min-w-0 flex-1 flex-col bg-bd-bg-chat text-bd-text-primary">
