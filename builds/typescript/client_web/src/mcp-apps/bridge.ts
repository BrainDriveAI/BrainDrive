...
function validLegacyDraft(value: unknown): value is LegacyMessageDraft { return isRecord(value) && exactKeys(value, ["bridge_version", "message_id", "type", "payload"]) && value.bridge_version === 1 && typeof value.message_id === "string" && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value.message_id) && typeof value.type === "string" && ["bridge.ready", "capability.discover", "capability.call", "export.request", "operation.cancel", "career.return", "host.action"].includes(value.type) && isRecord(value.payload); }
