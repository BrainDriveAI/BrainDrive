...
| `src/native_export.rs` | Native PDF/text Downloads-folder export, bounded request validation, non-overwriting atomic commit, and safe-label-only result |
...
The native first-party app runtime uses the same signed-package verifier, app-scoped operation journal, three-restart-per-installation budget, and per-app retention policy as Docker. Tauri stages the host runtime only, selects `desktop_windows_x64` on Windows or `desktop_macos_universal` on macOS, and enables the app platform below the platform data root. Apps, packages, and capability providers are catalog-installed owner selections, not initial desktop installer payload. Windows contains the backend tree in a kill-on-close Job Object with the existing one-logical-CPU-equivalent group ceiling and 512 MiB per-process ceiling. macOS launches each fixed backend child as a separate process group and terminates those groups on controlled shutdown, signal handling, or containment drop so Node descendants do not survive the shell. At most two first-party apps may be active. App processes bind authenticated random loopback and receive no inherited owner environment. Supervisor log lines are capped at 4 KiB and the file restarts at 1 MiB. The native export command writes validated PDF/text bytes to the operating-system Downloads directory using a browser-style non-overwriting filename and returns only the safe file label; renderer receipt finalization happens after that terminal outcome.

When no catalog override is provided, the packaged desktop gateway uses the BrainDrive-controlled Stage 1 catalog at `https://raw.githubusercontent.com/BrainDriveAI/BrainDrive-Marketplace/main/catalog/stage1/catalog.json`. `BRAINDRIVE_STAGE1_CATALOG_URL` and `BRAINDRIVE_STAGE1_CATALOG_PATH` remain authoritative overrides for development, candidate, and recovery runs.

The JavaScript and PowerShell staging paths both exclude extracted app and capability package payloads from the host installer and pass source-side preflight/tests. Those checks do not prove the changed Spec 08 candidate on either native platform. Fresh native Windows and macOS J-05 plus the two-app lifecycle/process/owner journey on the exact immutable candidate remain required; WSL/Linux diagnostics are not a substitute.

SC-008 defines extracted apps and capability providers as catalog-installed packages, not as initial desktop installer payload. The SearXNG proof package declares the `docker_linux_x64` container target plus `desktop_windows_x64` and `desktop_macos_universal` `packaged_process` targets. Desktop sidecar execution must use those packaged-process targets after the owner installs the package from the catalog, not Docker Desktop or Compose as a sidecar prerequisite for this proof provider. The sidecar package staging tool can build a real macOS Mach-O sidecar for catalog packaging; release distribution of that catalog artifact requires Developer ID codesigning and notarization for the downloaded package executable, separately from notarizing the BrainDrive host app.
...
- Rust shell, native Downloads export, or process containment: `cargo fmt --manifest-path Cargo.toml -- --check`, focused Cargo test, then `npm run desktop:test`.
