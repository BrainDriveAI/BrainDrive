...
SC-008 stages the Internet Search provider package manifest into the desktop runtime so Tauri can evaluate the same package/component metadata as Docker. The SearXNG proof package declares the `docker_linux_x64` container target plus `desktop_windows_x64` and `desktop_macos_universal` `packaged_process` targets. Desktop sidecar execution must use those packaged-process targets, not Docker Desktop or Compose as a sidecar prerequisite for this proof provider. Local macOS staging builds a real packaged-process Mach-O sidecar and records it as unsigned local evidence; release packaging still requires Developer ID codesigning and notarization for the app bundle and embedded sidecar, otherwise Gatekeeper/quarantine may block first launch outside a developer-controlled build tree.
