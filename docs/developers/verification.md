...
| Installed app package, including Resume Builder | Closest package test in `builds/resume_builder/` or the app package touched | From the package directory: `npm test`, `npm run build`; include runtime/web checks when host contracts or gateway/client integration changed | Manual app journey evidence only after the package version, descriptor, and host branch are frozen |
...
| `npm test` / `npm run build` | `builds/resume_builder/` | Installed Resume Builder package dependencies | None | Build emits package-local ignored output | Exit 0; classify package test/build; no tracked cleanup | A |
