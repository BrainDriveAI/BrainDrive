...
1. Freeze a clean immutable `SOURCE_TEST_REVISION`, run the source checks there, and bind it with `node tools/docs/candidate-digest.mjs --source-test-revision <full-sha>` from the repository root. The command emits `SOURCE_TEST_REVISION` and `SOURCE_CANDIDATE_PROOF`; the proof hashes the named commit tree while excluding only the fixed declared evidence-output paths. Later scorecard files cannot change that source proof. Do not collect evidence against a dirty or mutable source candidate.
...
The scorecard must include `SOURCE_TEST_REVISION`, `SOURCE_CANDIDATE_PROOF`, prompt, starting path, allowed context, prohibited-input confirmation, public-safe trace summary, required output, interventions, remaining risk, disposition, and sanitization. The later `EVIDENCE_REVISION` is the clean commit containing the scorecard and is supplied to or discovered by the checker; embedding an anticipated evidence commit SHA in the scorecard would be self-referential. Required maps, matrices, worksheets, comparisons, and handoffs must retain their substantive sanitized rows/items or exact repository references; a sentence claiming the artifact was retained is insufficient. Do not retain raw secrets, owner paths or data, private network identifiers, unrestricted logs, or hidden evaluator reasoning.

## Adjudication and rerun

The primary implementer verifies paths and commands independently, adjudicates each finding, makes the smallest authorized correction, and reruns every scenario whose mapped instruction, catalog route, source, check, boundary, rubric, or evidence contract changed between `SOURCE_TEST_REVISION` and `EVIDENCE_REVISION`. Agent-instruction or shared validator changes stale all affected scorecards; catalog, scenario, source, and check changes stale the scenarios mapped by the release checker. A scenario passes only when all of its binary gates pass without undocumented maintainer help, unsafe access, invented facts, or over-scoped changes.
