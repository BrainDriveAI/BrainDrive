...
When the Docker-development app platform is enabled, registered first-party apps use the separate [modern MCP Apps host](../../../builds/typescript/app-platform/mcp-host/README.md). Its route-selected adapter negotiates exact MCP/App versions, retains complete content/resource/annotation/metadata envelopes, reads only the exact verified manifest primary `ui://` resource, and brokers only same-server app-visible operations through an app/session/view-bound opaque-origin sandbox session. The current gateway composes explicit Resume Builder and Brief Builder adapters while synthetic registrations prove the generic host boundary. Model operations pass through the [protected inference capability](../../../builds/typescript/app-inference/README.md) into a reviewed app/purpose/version policy; they do not use static MCP config, register tools into the agent loop, expose credentials, endpoints, prompt material, or provider-selection authority to the app, or inherit the legacy permission-header fallback. The Resume-only result projection carries opaque provider/model and policy/digest lineage required by its immutable domain binding; the generic and Brief projections omit that lineage.
