...
Project file listing and reading also resolve registered app-published Markdown providers. The generic project service validates each publisher/logical identity, derives its reserved `published/<publisher-id>/<logical-id>.md` location, atomically refreshes changed bytes, and returns read-only source/display metadata beside ordinary project files. The existing web document viewer uses that metadata without knowing which app supplied the document. `PUT /projects/:id/file-content` returns a conflict for the reserved published namespace; the installed app never receives a path or general document-write capability. Resume Builder currently supplies the latest approved general resume to Career through this mechanism, independently of its narrow Career-return journal entry.
