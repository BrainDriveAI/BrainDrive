...
The Resume Builder Milestone 2 fixture is available for explicit authenticated lifecycle exercises, but it does not install or start automatically. Its supervised endpoint binds only inside the app container and Compose publishes no additional port. `stop.sh dev` preserves its named volume; do not remove that volume unless the exact lifecycle test state is task-owned and destructive cleanup is explicitly authorized.
