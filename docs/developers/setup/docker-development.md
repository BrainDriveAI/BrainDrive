...
| Internet Search sidecar | Private Compose service for local owner-managed SearXNG V1 when Internet Search is enabled; it is exposed only on the Compose network and the app calls it through generic `web.search@1` / `web.read@1` Host routes |
| Network | Compose network `braindrive_dev_default`; app and the Internet Search sidecar remain internal while Vite exposes the selected host bind and proxies `/api` to the app |
