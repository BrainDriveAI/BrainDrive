...
- Execute and score provider-independent coding-agent scenarios only through [`developers/verification/ai-agent-harness.md`](developers/verification/ai-agent-harness.md). AI output is read-only evaluation evidence, never human or area-owner review.
- Before specifying, planning, or implementing documentation work, follow the root `AGENTS.md` context-discovery sequence: select the applicable catalog task route, read only the routed canonical documentation and executable evidence, record material governing paths, and stop on material conflicts. Do not recursively ingest the entire documentation tree.
