...
if [[ ! -t 0 && -z "${COSIGN_PASSWORD:-}" ]]; then
  echo "Non-interactive shell detected and COSIGN_PASSWORD is not set." >&2
  echo "If ${KEY_PATH} is passphrase-protected, set COSIGN_PASSWORD before signing." >&2
fi
