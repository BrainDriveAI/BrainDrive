# Shared signed-release manifest resolution for first installs and upgrades.
# Callers must define $rootDir and Get-EnvValue before dot-sourcing this file.
...
function Ensure-Cosign {
  ...
  if (-not $version) {
    $version = $script:BrainDriveEmbeddedCosignVersion
  }
  if ($version -ne $script:BrainDriveEmbeddedCosignVersion) {
    throw "Automatic cosign install only supports pinned version $script:BrainDriveEmbeddedCosignVersion; requested $version. Install another version manually and set BRAINDRIVE_COSIGN_BIN."
  }
  ...
  $url = "https://github.com/sigstore/cosign/releases/download/$version/cosign-$platform-$arch$ext"
  $expectedSha256 = Get-BrainDriveCosignSha256 -Platform $platform -Architecture $arch

  Write-Host "cosign not found; downloading $url"
  try {
    Invoke-WebRequest -Uri $url -OutFile $tmpTarget
    Test-BrainDriveSha256 -Path $tmpTarget -ExpectedSha256 $expectedSha256 -Label "Downloaded cosign $version ($platform-$arch)"
